...
class BatchI18nUpdater {
    constructor(baseDir) {
        ...
        this.footerPattern = /<p[^>]*>&copy; 2025 Legacy Concierge\. All Rights Reserved\.<\/p>/g;
    }
    ...
    updateFooter(content) {
        return content.replace(this.footerPattern, 
    ...
    addI18nScript(content, isSubDirectory = false) {
        const scriptPath = isSubDirectory ? '../scripts/i18n.js' : 'scripts/i18n.js';
    ...
    processFile(filePath) {
        try {
            ...
            const isSubDirectory = filePath.includes('/treatments/') || filePath.includes('/expertise/');
    ...
    generateContentMapping(filePath) {
        ...
        if (isSubDirectory) {
            const type = filePath.includes('/treatments/') ? 'treatments' : 'expertise';
            return {

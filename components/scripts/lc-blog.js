...
class LCBlogMeta extends HTMLElement {
    ...
    hideEmptyMetaItems() {
        ...
        const items = meta.querySelectorAll('.lc-blog-meta-author, .lc-blog-meta-date, .lc-blog-meta-reading-time');
...
class LCBlogFilter extends HTMLElement {
    ...
    filterBySearch(query, target) {
        ...
        const cards = container.querySelectorAll('lc-blog-card, .lc-blog-card, [data-searchable]');

        for (const card of cards) {
            const title = card.querySelector('[slot="title"], .lc-blog-card-title')?.textContent.toLowerCase() || '';
            const excerpt = card.querySelector('[slot="excerpt"], .lc-blog-card-excerpt')?.textContent.toLowerCase() || '';
        ...
        this.dispatchEvent(new CustomEvent('lc-filter-search', {
            detail: { query },
            bubbles: true
        }));
    }
    ...
    filterByCategory(category, target) {
        ...
        const cards = container.querySelectorAll('lc-blog-card, .lc-blog-card, [data-category]');
        ...
        this.dispatchEvent(new CustomEvent('lc-filter-category', {
            detail: { category },
            bubbles: true
        }));
    }

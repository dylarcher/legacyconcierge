...
class LCDialog extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && !getBooleanAttribute(this, 'no-escape-close')) {
                this.close();
            }
        });
...
class LCTooltip extends HTMLElement {
    connectedCallback() {
        ...
        const triggerElement = trigger?.querySelector('[slot="trigger"]') || trigger?.firstElementChild;
        if (triggerElement) {
...
class LCPopover extends HTMLElement {
    ...
    open() {
        ...
        const trigger = this.querySelector('.lc-popover-trigger [slot="trigger"]') ||
...
class LCAlert extends HTMLElement {
    ...
    close() {
        ...
        if (alert) {
            ...
            setTimeout(() => {
                this.remove();
                this.dispatchEvent(new CustomEvent('lc-alert-close', { bubbles: true }));
            }, 300);
        } else {
            this.remove();
        }

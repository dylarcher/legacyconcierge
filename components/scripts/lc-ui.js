...
import {
  getAttributeOr,
  getBooleanAttribute,
} from "/scripts/core/component-loader.js";
...
class LCPagination extends HTMLElement {
  ...
  render(current, total, showFirstLast) {
    ...
    // Previous button
    ul.appendChild(
      this.createButton("‹", current - 1, current === 1, "Previous page"),
    );
    ...
    // Next button
    ul.appendChild(
      this.createButton("›", current + 1, current === total, "Next page"),
    );

    // Last button
    if (showFirstLast) {
      ul.appendChild(
        this.createButton("»", total, current === total, "Last page"),
      );
    }
  ...
  goToPage(page) {
    ...
    this.dispatchEvent(
      new CustomEvent("lc-page-change", {
        detail: { page },
        bubbles: true,
      }),
    );
...
export { LCBadge, LCTag, LCButton, LCPagination, LCSpinner };

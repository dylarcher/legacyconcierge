...
import { getAttributeOr, getBooleanAttribute } from '../../scripts/core/helpers.js';
...
class LCCard extends HTMLElement {
    connectedCallback() {
        ...
        existingContent.forEach(node => this.appendChild(node));
...
class LCCardGrid extends HTMLElement {
    connectedCallback() {
        ...
        cards.forEach(card => slot.parentNode.insertBefore(card, slot));

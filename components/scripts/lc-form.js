...
import { cloneTemplate, getBooleanAttribute, getAttributeOr } from '/scripts/core/component-loader.js';
...
class LCForm extends HTMLElement {
    connectedCallback() {
        ...
        const children = Array.from(this.childNodes).filter(node =>
            node !== form && node.nodeType === Node.ELEMENT_NODE
        );
    ...
    handleSubmit(event) {
        if (getBooleanAttribute(this, 'validate-on-submit')) {
            ...
            if (this.validateAll()) {
                ...
                this.dispatchEvent(new CustomEvent('lc-form-submit', {
     ...
     */
    validateAll() {
        const elements = this.querySelectorAll('lc-input, lc-textarea, lc-select, lc-checkbox, lc-radio-group');
        let isValid = true;
...
class LCInput extends LCFormElement {
    connectedCallback() {
        ...
        ['min', 'max', 'minlength', 'maxlength', 'pattern', 'step'].forEach(attr => {
        ...
        input.addEventListener('input', () => {
            this.dispatchEvent(new CustomEvent('lc-input', {
...
class LCTextarea extends LCFormElement {
    connectedCallback() {
        ...
        // Set wrapper attributes
        wrapper.setAttribute('data-required', String(required));
        wrapper.setAttribute('data-show-counter', String(getBooleanAttribute(this, 'show-counter')));
...
class LCSelect extends LCFormElement {
    connectedCallback() {
        ...
        select.addEventListener('change', () => {
            this.validate();
            this.dispatchEvent(new CustomEvent('lc-change', {
...
class LCRadioGroup extends LCFormElement {
    ...
    validate() {
        const required = this.querySelector('.lc-radio-group').getAttribute('data-required') === 'true';
...
class LCRadio extends HTMLElement {
    connectedCallback() {
        ...
        // Setup change event
        input.addEventListener('change', () => {
            this.dispatchEvent(new CustomEvent('lc-radio-change', {
                detail: { value: input.value },
                bubbles: true
            }));
        });
